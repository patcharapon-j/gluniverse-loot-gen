...
- **Workshop gear is etched too**: when the Loot Workshop authors a magic weapon/armor, the LLM also returns a `runes` block (potency, striking/resilient, property slugs). `sanitizeRuneSet` validates it against the same table — clamping tiers, enforcing the striking⇒potency and property⇒potency RAW prerequisites, and dropping any rune that fails its Usage restriction or is unknown to the live system. The authored price is split into a derived base + rune cost (`runePriceOf`) so the sheet total ≈ the model's fair value and runes are never double-counted. If the model tags an item magical but omits runes, a level-appropriate themed set is derived as a fallback.
...
- **Concept-driven stock (LLM "buyer", opt-in).** The GM's free-text concept ("black-market potion dealer in the sewers") becomes a `/shop-stock` call that returns a **selection profile** — item-type mix, trait weights, a rarity lean, a list of **named items** the shop would carry, and exclusions. The *code* then resolves it: each named item is fuzzy-matched to a **real, priced, level-bounded compendium entry** (`resolveWanted`), and the remaining slots are filled by the existing weighted selector now steered by the profile's trait/type/rarity weights. The model curates *kinds*; it never names UUIDs, sets prices, or invents items here. Per the design knobs: a brief can lean the shelf toward **restricted (uncommon/rare)** goods, but item **levels stay bounded by the tier's reach**, and anything the model names that doesn't resolve to a real ≤-cap item is silently dropped. Null profile (no concept / no sidecar / failure) → falls back to pure tier-theme selection.
- **LLM flavor layer (opt-in, same §14 contract — flavor only, fails closed).** One batched `/shop` call returns a **shopkeeper persona** (name, sign, greeting, bio → written to the actor's description) **and** per-item provenance. **Signature stock** is authored through the existing `/workshop` path (reusing all its sanitization/pricing), idempotently, so a shop carries 1–2 bespoke items it's "known for" — the only items genuinely *invented* (DESIGN choice: curate real + few bespoke). Code always decides the stock, prices, and legality; the model only dresses it. Any failure leaves the shop fully usable with plain rules-text.
